...
(function($){
    $.fn.yxMobileSlider = function(settings){
        ...
        return this.each(function(){
            ...
            timer_of_touch=0;  //触摸时间，用于判断是否为点击事件
            //初始化主体样式
...
//                background: 'rgba(0,0,0,0.5)'
            ...
            $(window).bind('resize load', function(){
               ...
               lastbtn.bind('click',lastpage);
               nextbtn.bind('click',nextpage);
            });
            //页面加载完毕BANNER自动滚动
            autoMove();
            //PC机下焦点切换
            if (!isMobile()) {
             ...
             function lastpage() {
                ...
                else if(iCurr==num-1)
                {
                 ...
                 doAnimate(-iCurr * moveWidth, autoMove);
                 pageno.html((iCurr+1).toString());
             }


             function nextpage() {
                 if(iCurr<num-1) iCurr++;
                 if(iCurr==0) 
                    {
                      lastbtn.css("display","none");
                      nextbtn.css("display","");    
...
function DomReload(){
               ...
               num = oLi.length; //滚动屏幕数
                ...
                iCurr=0;
                pageno.html((iCurr+1).toString());
                pagetotal.html(oLi.length.toString());
                btnupdate(iCurr,num);

}
function btnupdate(iCurr,totalpage){
    if(iCurr==0)
    {
        lastbtn.css("display","none");
       if(totalpage==1) nextbtn.css("display","none");
       else             nextbtn.css("display","");
    }
    else if(iCurr==totalpage-1)
    {
        lastbtn.css("display","");
        nextbtn.css("display","none");
    }
